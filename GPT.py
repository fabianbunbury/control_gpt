# this example is taken from https://jaketae.github.io/study/gpt/
import math
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        value = self.value(x).reshape(batch_size, seq_len, self.num_heads, -1).transpose(1, 2)
        query = self.query(x).reshape(batch_size, seq_len, self.num_heads, -1).transpose(1, 2)
        # shape == (batch_size, num_heads, seq_len, head_dim)
        
        attn = torch.matmul(query, k_t) / math.sqrt(query.size(-1))
